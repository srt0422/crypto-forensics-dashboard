...
import { inboundTransactions } from '@/data/mockData';
...
const InboundTransactions = () => {
  ...
  const chains = useMemo(() => {
    const uniqueChains = new Set(inboundTransactions.map(tx => tx.chain));
    return Array.from(uniqueChains);
  }, []);

  // Apply filters
  const filteredTransactions = useMemo(() => {
    return inboundTransactions.filter(tx => {
      // Chain filter
  ...
  return (
    <div>
      <DashboardHeader 
        ...
        subtitle="Detailed view of all incoming transactions" 
      />
              ...
              <tbody>
                {filteredTransactions.map((tx) => (
                  <tr key={tx.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 font-medium text-crypto-green">
                      +${tx.amount.toFixed(2)}
                    </td>
                    <td className="py-3 px-4">
